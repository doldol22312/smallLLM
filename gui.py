...
class TinyLLMGUI:
    ...
    def setup_train_tab(self):
        ...
        self.add_option(train_group, "LR schedule:", "lr_schedule", ["constant", "cosine", "wsd"], 4)
        self.add_entry(train_group, "LR warmup steps:", "lr_warmup_steps", "200", 5)
        self.add_entry(train_group, "LR stable steps (WSD):", "lr_stable_steps", "0", 6)
        self.add_entry(train_group, "LR min:", "lr_min", "3e-5", 7)
        self.add_option(train_group, "Optimizer:", "optimizer", ["adamw", "adamw8bit", "lion"], 8)
        self.add_entry(train_group, "Lion beta1:", "lion_beta1", "0.9", 9)
        self.add_entry(train_group, "Lion beta2:", "lion_beta2", "0.99", 10)
        self.add_entry(train_group, "Weight decay:", "weight_decay", "0.1", 11)
        ...
        self.add_entry(perf_group, "DataLoader workers:", "dataloader_workers", "0", 6)
        self.add_entry(perf_group, "Prefetch factor:", "dataloader_prefetch_factor", "2", 7)
        self.add_check(perf_group, "Persistent workers", "dataloader_persistent_workers", True, 8)
    ...
    def start_training(self):
        ...
        cmd.extend(["--dataloader_workers", self.vars["dataloader_workers"].get()])
        cmd.extend(["--dataloader_prefetch_factor", self.vars["dataloader_prefetch_factor"].get()])
        if self.vars["dataloader_persistent_workers"].get(): cmd.append("--dataloader_persistent_workers")
        else: cmd.append("--no-dataloader_persistent_workers")
        ...
        cmd.extend(["--lr_stable_steps", self.vars["lr_stable_steps"].get()])
        ...
        cmd.extend(["--ema_update_every", self.vars["ema_update_every"].get()])
        cmd.extend(["--ema_start_step", self.vars["ema_start_step"].get()])

        if self.vars["curriculum"].get(): cmd.append("--curriculum")
        else: cmd.append("--no-curriculum")
        cmd.extend(["--curriculum_start_block_size", self.vars["curriculum_start_block_size"].get()])
        cmd.extend(["--curriculum_steps", self.vars["curriculum_steps"].get()])
